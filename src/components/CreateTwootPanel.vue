<template>
    <form class="create-twoot-panel" @submit.prevent="createNewTwoot" :class="{'--exceeded' : newTwootCharacterCount > 180}">
        <label for="newTwoot"><strong>New Twoot</strong> ({{ newTwootCharacterCount }}/180)</label>
        <textarea id="mewTwoot" rows="4" v-model="state.newTwootContent" />
        
        <div class="create-twoot-panel__submit">
            <div class="create-twoot-type">
                <label for="newTwootType"><strong>Type</strong></label>
                <select id="newTwootType" v-model="state.selectedTwootType">
                    <option :value="option.value" v-for="(option, index) in state.twootTypes" :key="index">
                        {{ option.name }}
                    </option>
                </select>
            </div>
            <button>
                Twoot!
            </button>
        </div>
    </form>
</template>

<script>
import { reactive, computed } from 'vue';

export default {
    name: "CreateTwootPanel",

    //old way to do (Vue 2)
    // data() {
    //     return {
    //         newTwootContent: '', //v-model to save into here
    //         selectedTwootType: 'instant', //eg of initialize by default as this is synced with dropdown
    //         twootTypes: [
    //             {value: 'draft', name: 'Draft'},
    //             {value: 'instant', name: 'Instant Twoot'}
    //         ]
    //     }
    // },
    // computed: {
    //     newTwootCharacterCount() {
    //         return this.newTwootContent.length;
    //     }
    //     // fullName() {
    //     //     return `${this.user.firstName} ${this.user.lastName}`;
    //     //     //this.user.firstName + ' ' + this.user.lastName #this is the same thing
    //     // }
    // },
    // methods: {
    //     createNewTwoot() {
    //         if (this.newTwootContent && this.selectedTwootType !== "draft") {
    //             this.$emit('add-twoot', this.newTwootContent);
    //             this.newTwootContent = '';
    //         }
    //     }
    // }

    //Vue 3 new composition API  -- basically a refactor of above
    setup(props, ctx) {
        const state = reactive({
           newTwootContent: '', //v-model to save into here
            selectedTwootType: 'instant', //eg of initialize by default as this is synced with dropdown
            twootTypes: [
                {value: 'draft', name: 'Draft'},
                {value: 'instant', name: 'Instant Twoot'}
            ] 
        })

        const newTwootCharacterCount = computed(() => state.newTwootContent.length)

        function createNewTwoot() {
            if (state.newTwootContent && state.selectedTwootType !== "draft") {
                ctx.emit('add-twoot', state.newTwootContent);
                state.newTwootContent = '';
            }
        }

        //in componation API we always need to return everything that we state above
        return {
            state,
            newTwootCharacterCount,
            createNewTwoot
        }
    }
}
</script>

<style lang="scss" scoped>
.create-twoot-panel {
    margin-top: 20px;
    padding:  20px 0;
    display: flex;
    flex-direction: column;

    textarea {
        border: 1px solid #dfe3e8;
        border-radius: 5px;
    }

    .create-twoot-panel__submit {
        display: flex;
        justify-content: space-between;

        .create-twoot-type {
            padding: 10px 0;
        }

        button {
            padding: 5x 20px;
            margin: auto 0;
            border-radius: 5px;
            border: none;
            background-color: deeppink;
            color: white;
            font-weight: bold;
        }
    }

    &.--exceeded {
        color: red;
        border-color: red;

        .create-twoot-panel__submit {
            button {
                background-color: red;
                color: white;
            }
        }
    }
}
</style>